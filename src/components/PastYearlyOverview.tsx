...
interface PastYearlyOverviewProps {
  clients: Client[];
...
export function PastYearlyOverview({ clients, onMonthClick }: PastYearlyOverviewProps) {
    ...
    <div className="bg-white rounded-3xl shadow-3d hover:shadow-3d-hover transition-all duration-500">
      <div className="p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <h2 className="text-[#2D1B69] font-semibold text-xl lg:text-2xl">
            <span className="sm:hidden">Обзор платежей за прошлые пол года</span>
            <span className="hidden sm:inline">Обзор платежей за прошлый год</span>
          </h2>
          <div className="flex items-center gap-4 sm:gap-6 text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-600">Ожидали</span>
            </div>
          </div>
        </div>
        ...
        <div className="block sm:hidden">
          <div className="grid grid-cols-6 gap-1.5">
            {monthsData.slice(-6).map((month, index) => {
              ...
              return (
                <div key={index} className="flex flex-col items-center">
                  <div 
                    className="w-full h-20 flex flex-col justify-end mb-1 cursor-pointer"
                    onClick={() => onMonthClick(month.date)}
                  >
                    {month.totalAmount > 0 ? (
                      <div 
                        className="w-full flex flex-col rounded-t-lg overflow-hidden shadow-md"
                      ...
                      <div className="w-full h-3 bg-gray-100 rounded-lg opacity-50" />
                    )}
                  </div>
                  <div className="text-center w-full">
                    <p className="text-gray-900 text-[10px] font-medium">{getMonthName(month.date)}</p>
                    {month.count > 0 && (
                      <div className="bg-gray-50 rounded-md p-1 mt-0.5">
          ...
          <div className="grid grid-cols-12 gap-3">
            {monthsData.map((month, index) => {
              ...
              return (
                <div key={index} className="flex flex-col items-center">
                  <div 
                    className="w-full h-48 flex flex-col justify-end mb-3 relative group cursor-pointer"
                    onClick={() => onMonthClick(month.date)}
                  ...
                  <div className="text-center w-full space-y-1">
                    <p className="text-gray-900 text-sm">{getMonthName(month.date)}</p>
                    {month.count > 0 && (
                      <>
                        <p className="text-gray-500 text-xs">{month.count} шт</p>
                        ...
                        {month.percentage < 100 && (
                          <p className="text-red-500 text-xs mt-1">{month.percentage}%</p>
                        )}
                      </>
                    )}
                  </div>
